...
const App = () => {
  ...
  // 初始化
  useEffect(() => {
    const savedData = localStorage.getItem('seatingChartData_v14'); // 更新版本號
    if (savedData) {
      try {
  ...
  useEffect(() => {
    if (tables.length > 0) {
      localStorage.setItem(
        'seatingChartData_v14',
        JSON.stringify({ members, tables })
      );
    }
  }, [members, tables]);
  ...
  const handleQuickAddSubmit = (tableIndex, tableId) => {
    ...
    if (isAlreadySeated) {
      if (
        !window.confirm(
          `${member.name} (編號 ${member.id}) 已經在其他座位了，確定要換到這裡嗎？`
        )
      )
        return;
    }
    ...
    if (selectedMember && !currentOccupant) {
      ...
      setTables((prevTables) => {
        const tablesAfterRemoval = prevTables.map((t) => ({
          ...t,
          seats: t.seats.map((s) =>
            s && s.id === selectedMember.id ? null : s
          ),
        }));
            ...
            0 ? (
              ...
              tables
                ...
                .map((table) => (
                  <div
                    key={table.id}
                    className="bg-white w-full aspect-[1/1.414] shadow-xl mb-8 print:shadow-none print:mb-0 print:break-after-page flex flex-col p-12 relative overflow-hidden box-border"
                  >
                    {/* 頁面邊框裝飾 */}
                    <div className="absolute inset-4 border-4 border-double border-gray-300 pointer-events-none rounded-xl print:border-gray-400"></div>

                    {/* 上半部：正面 (折疊後立起面向外) - 桌號大字 */}
                    <div className="flex-1 flex flex-col justify-center items-center border-b-2 border-dashed border-gray-400 mb-4 pb-4">
                      <div className="text-center transform scale-125 origin-center">
                        <h1 className="text-[6rem] font-black leading-tight tracking-tight text-gray-900 mb-4">
                          {table.name}
                        </h1>
                        {table.type === 'vegetarian' && (
                          <div
                            className="flex items-center justify-center gap-3 px-8 py-3 bg-green-100 text-green-800 rounded-full border-4 border-green-500 mt-2 print:bg-green-100 print:text-green-800 print:border-green-500"
                            style={{
                          ...
                          >
                            <Leaf size={40} />
                            <span className="text-4xl font-bold">素食桌</span>
                          </div>
                        )}
                      </div>
                      <div className="mt-auto text-gray-400 flex items-center gap-2 text-sm pt-8">
                        <Scissors size={14} />
                        <span>--- 此線可對折 (山折) ---</span>
                      </div>
                    </div>

                    {/* 下半部：背面 (折疊後立起面向內/賓客) - 名單 */}
                    <div className="flex-1 pt-10 px-8">
                      <h2 className="text-4xl font-bold text-center mb-10 text-gray-800 border-b-4 border-gray-800 pb-4 inline-block w-full">
                        賓客名單
                      </h2>
                      <div className="grid grid-cols-2 gap-x-16 gap-y-8">
                        {table.seats.map((seat, idx) => (
                          <div
                            key={idx}
                            className="flex items-end text-3xl border-b border-gray-300 py-2"
                          >
                            <span className="w-12 text-gray-400 text-2xl font-mono mr-2 mb-1">
                              {idx + 1}.
                            </span>
                            <span
                              className={`font-bold tracking-wider text-4xl truncate ${
                                seat ? 'text-black' : 'text-gray-200'
                              }`}
                            >
                              {seat ? seat.name : ''}
                            </span>
                      ...
                      <div className="mt-auto pt-12 text-center text-gray-400 font-medium">
                        長青會聚餐座位表
                      </div>
                    </div>
                  </div>
          ...
          <div className="p-4 border-b bg-gray-50">
            <h2 className="font-bold text-gray-700 mb-3 flex justify-between items-center">
              ...
              <span className="text-xs font-normal text-gray-500 bg-gray-200 px-2 py-0.5 rounded-full">
                {seatedMemberIds.size} / {members.length} 入座
              </span>
            </h2>
            ...
            {!showAddMember ? (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2 bg-emerald-100 text-emerald-700 rounded-lg hover:bg-emerald-200 transition text-sm font-medium"
              >
                <UserPlus size={16} /> 新增會員
              </button>
            ) : (
              <form
                onSubmit={handleAddMember}
                className="bg-emerald-50 p-2 rounded-lg border border-emerald-200 shadow-sm animate-fade-in"
              >
                <div className="flex justify-between items-center mb-2 text-xs text-emerald-800 font-bold">
                  <span>新增會員</span>
                  <button
                    type="button"
                ...
                <div className="flex gap-2 mb-2">
                  <input
                    type="text"
                    className="w-1/3 px-2 py-1.5 border rounded text-sm outline-none"
                    value={newMemberId}
                  ...
                  <input
                    ref={nameInputRef}
                    type="text"
                    className="w-2/3 px-2 py-1.5 border rounded text-sm outline-none"
                    value={newMemberName}
                ...
                <button
                  type="submit"
                  className="w-full bg-emerald-600 text-white py-1.5 rounded text-sm hover:bg-emerald-700 flex items-center justify-center gap-1"
                >
                  <Check size={14} /> 確認
                </button>
              </form>
            ...
            {filteredMembers.map((member) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between p-2 mb-1.5 rounded-lg cursor-pointer transition select-none ${
                    isSeated
